...
function init(){
	...
	var confmKey = "devU01TX0FVVEgyMDIyMDcxNjE5MjAyMjExMjc5NjI=";//승인키
	// resultType항목 추가(2016.10.06)
	var resultType = "4"; // 도로명주소 검색결과 화면 출력유형, 1 : 도로명, 2 : 도로명+지번, 3 : 도로명+상세건물명, 4 : 도로명+지번+상세건물명
	var inputYn= "<%=inputYn%>";
	if(inputYn != "Y"){
		document.joinFrm.confmKey.value = confmKey;
		document.joinFrm.returnUrl.value = url;
		document.joinFrm.resultType.value = resultType; // resultType항목 추가(2016.10.06)
		document.joinFrm.action="https://www.juso.go.kr/addrlink/addrLinkUrl.do"; // 인터넷망
